/*
11. Define a Binary_tree class derived from Shape. Give the number of levels as
a parameter (levels==0 means no nodes, levels==1 means one node, levels==2 means
one top node with two sub-nodes, levels==3 means one top node with two sub-nodes
each with two sub-nodes, etc.). Let a node be represented by a small circle.
Connect the nodes by lines (as is conventional). P.S. In computer science, trees
grow downward from a top node (amusingly, but logically, often called the root).
13. Modify Binary_tree to take a parameter (or parameters) to indicate what kind
of line to use to connect the nodes (e.g., an arrow pointing down or a red arrow
pointing up). Note how this exercise and the last use two alternative ways of
making a class hierarchy more flexible and useful.
14. Add an operation to Binary_tree that adds text to a node. You may have to
modify the design of Binary_tree to implement this elegantly. Choose a way to
identify a node; for example, you might give a string "lrrlr" for navigating
left, right, right, left, and right down a binary tree (the root node would
match both an initial l and an initial r).
*/
...
public:
  Binary_tree(int llevels, Line_style::Line_style_type llstyle)
      : levels{llevels}, lstyle{llstyle} {
    add_nodes();
    add_lines();
  }
...
private:
  ...
  vector<pair<string, string>>
      path_text; // pairs: Path to node in format "llrl"(root->left->left...),
                 // node text
};
...
void Binary_tree::add_lines() {
  int height{4 * r};
  int width{};
  for (int i = 0; i < pow(2, levels - 1) - 1;
       ++i) { // iterate levels excluding last level
    width = 12 * r / (i + 1);
